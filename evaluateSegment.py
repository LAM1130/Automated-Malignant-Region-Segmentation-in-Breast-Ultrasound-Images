# -*- coding: utf-8 -*-
"""
Created on Sun Aug 20 03:57:37 2024
...
onlyfiles = [f for f in listdir(input_dir) if isfile(join(input_dir, f))and f.lower().endswith(('.png',))]
...
for i,name in enumerate(files):
    ...
    plt.imsave(output_dir + '/' + imgName[0] + '_output.png',255*outputImg,cmap=cm.gray)#changed file name, then outImg/5 deleted
    gt = cv2.imread(groundtruth_dir + '/' + imgName[0] +'_mask'+ '.png',0)#changed
    gt = np.round(gt.astype('float32')/255)#changed no 5*gt
...
if verbose==1:
    ...
    pt = ['Background','Malignant']
    avg_error = np.mean(error,axis=0)               #changed axis change to 0
    avg_precision = np.mean(precision,axis=0)
    avg_recall = np.mean(recall,axis=0)
    avg_iou = np.mean(iou,axis=0)
    
    for i in range(2):                                              #changed range change to 2 and /numImages change to /2
